...
export default function ServicesSection() {
  return (
    <div className="px-16 py-12 bg-[#FAF8ED] my-16">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {/* Right Cards */}
        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 px-32">
          {/* Notifications Card */}
            <div className="border border-[#034833] rounded-xl p-6 bg-white flex flex-col justify-between">
            <div>
                <h3 className="text-[32px] font-semibold text-[#034833] tracking-tighter">Notifications</h3>
                <p className="text-[54px] font-bold mt-2 text-[#034833]">$99<span className="font-normal text-[#5B5F62] text-[20px]">/monthly</span></p>
                <p className="text-[16px] text-[#666] mt-1">Best for families and travelers on a budget</p>

                <div className="space-y-4 my-8">
                <div className="flex flex-row items-center gap-4">
                    <img src={cash} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">3–4 times cheaper than Schengen Visa agents</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={calendar} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">You book an appointment on the next week</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={family} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">1 subscription for all travelers</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={clock} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">1 week – average time for booking an appointment</p>
                </div>
                </div>
            </div>
            <button className='relative flex space-x-4 px-8 py-2 bg-[#F66F4D] h-[70px] rounded-full text-[#FAF8ED] font-bold items-center justify-center'>
                <span className='pr-12'>GET APPOINTMENT ALERT</span>
                <ArrowUpRight className='stroke-white border-2 rounded-full h-[52px] w-[52px] absolute right-2' />
            </button>
            </div>
            ...
            <div>
                ...
                <div className="flex flex-row items-center gap-4">
                    ...
                    <p className="text-sm text-[#333]">Suitable if you can’t respond right away to notifications</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={halfmoon} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">We book a slot for you, even when you sleep</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={smile} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">You won’t have to think about booking—we’ll do everything</p>
                </div>
                <div className="flex flex-row items-center gap-4">
                    <img src={plusperson} alt="bell icon" className="h-[42px] w-[42px]" />
                    <p className="text-sm text-[#333]">Can book a slot even for the next day</p>
                </div>
                </div>
            </div>
            <button className='relative flex space-x-4 px-8 py-2 bg-[#F66F4D] h-[70px] rounded-full text-[#FAF8ED] font-bold items-center justify-center'>
